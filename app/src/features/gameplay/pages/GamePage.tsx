			...
			<div className="grid grid-cols-3 w-full justify-center gap-4">
					...
					<div className="flex flex-row w-max p-1 rounded-md shadow-md shadow-gray-400">
						<div className="flex flex-col">
							{Object.entries(whitePieceIcons).map(([abbreviation, icon]) => (
								<button key={abbreviation} type="button" className="hover:bg-gray-400">
						...
						<div className="flex flex-col">
							{Object.entries(blackPieceIcons).map(([abbreviation, icon]) => (
								<button type="button" key={abbreviation} className="hover:bg-gray-400">
									<img className="w-12 h-12" src={icon} alt={abbreviation} />
								</button>
							))}
						</div>
				...
				<div className="flex flex-col justify-center">
					<div className="flex flex-col shadow-gray-400 shadow-md w-max p-1 gap-2 rounded-md">
						<button aria-label="Eraser mode (remove a piece)" type="button" className="hover: bg-gray-400">
							<Eraser />
						</button>

						<button aria-label="Clear board" type="button" className="hover: bg-gray-400">
							<TrashCan />
						</button>

						<button aria-label="Flip board" type="button" className="hover: bg-gray-400">
							<Flip />
						</button>
					</div>
				</div>
