						...
						return (
							...
							>
								{isSquareOnLeftEdge(file, "white") && (
									<span className={`absolute top-1 left-1 text-sm font-bold ${isDark ? "text-gray-100" : "text-gray-400"}`}>{rank}</span>
								)}

								{isSquareOnBottomEdge(rank, "white") && (
									<span className={`absolute right-1 bottom-0.5 text-sm font-bold ${isDark ? "text-gray-100" : "text-gray-400"}`}>{file}</span>
								)}
							</div>
						)
					})
